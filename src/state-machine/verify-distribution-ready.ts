...
/**
 * Handles state machine step for validating that a CloudFront Distribution is ready
 */
export async function verifyDistributionReady(distId: string): Promise<void> {
    let distStatus = "";
    ...
    if (!distEnabled || distStatus !== "Deployed") {
        throw new Error(`Expected distribution to be enabled and Deployed, but is currently ${distEnabled ? "enabled" : "disabled"} and ${distStatus}`);
    }
    ...
    try {
        ...
        dist.status = DistributionStatus.Active;
